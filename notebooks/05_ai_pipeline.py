# ============================================================
# MODULE 5: AI-ACCELERATED DATA PIPELINE
# ============================================================
# What you'll learn:
#   - How to call the OpenAI / Anthropic API programmatically
#   - How to build a LangChain agent for automated EDA
#   - How to generate synthetic data
#   - How to build an end-to-end automated pipeline
#   - How to generate LLM-written insight reports
#
# CONCEPTS EXPLAINED:
#   API = Application Programming Interface
#       → A way to send requests to an AI model and get responses
#   LangChain = A Python library for building AI agents
#   Agent = An AI that can decide which tools/steps to use
#   Synthetic data = Fake-but-realistic data generated by an algorithm
...
# ══════════════════════════════════════════════════════════
# PART A: AUTOMATED EDA SUMMARY (No API key needed)
# ══════════════════════════════════════════════════════════
# This part runs without any API key.
# It auto-generates a structured EDA summary dict that you'd
# then pass to an LLM for narrative generation.
print("\n─────────────────────────────────────────────────────")
...
def generate_eda_summary(df: pd.DataFrame) -> dict:
    """
    Auto-generate a structured EDA summary from any DataFrame.
    
    This is the kind of function you'd run in a pipeline to
    automatically describe new data without manual analysis.
    
    Returns a dict that can be:
    - Passed to an LLM to write a report
    - Logged for monitoring
    - Compared against yesterday's summary to detect drift
    """
    summary = {}
...
if not api_key:
    ...
    EXAMPLE_PROMPT = f"""
You are a Senior Data Scientist presenting findings to a trading desk.
Analyze this dataset summary and write a professional executive report.
...
"""
    print("  ─── EXAMPLE PROMPT (what we'd send to GPT-4) ───")
...
if llm_available:
    try:
        ...
        prompt = f"""
You are a Senior Data Scientist presenting findings to a trading desk.
Analyze this retail dataset summary and write a professional executive report.
...
#   - Decide which analysis to run based on a user question
#   - Call Python functions as "tools"
...
# Each tool is a Python function with a clear docstring
# The agent reads the docstring to decide when to use it

def tool_top_products(n: int = 5) -> str:
    """Return top N products by profit. Use when asked about best products."""
...
def tool_discount_analysis() -> str:
    """Analyze the impact of discounts on profit. Use when asked about discounts."""
...
def tool_market_summary() -> str:
    """Summarize performance by market. Use for geographic or market questions."""
...
def tool_detect_anomalies() -> str:
    """Detect anomalous or unusual orders. Use to find outliers or data issues."""
    flags = []
...
def simple_agent(question: str) -> str:
    """
    A simple rule-based agent that chooses the right tool based on keywords.
    
    In a real LangChain setup, an LLM would choose the tool.
    This version uses keyword matching as a lightweight alternative.
    """
...
# ══════════════════════════════════════════════════════════
# Synthetic data = realistic fake data generated from patterns in real data
# Used to:
#   - Test models on rare scenarios (very high discounts, etc.)
#   - Augment small datasets
#   - Share data safely (no real customer info)
print("\n─────────────────────────────────────────────────────")
...
# ══════════════════════════════════════════════════════════
# This simulates what a production pipeline looks like.
# In production, you'd use Airflow or Prefect to schedule this.
print("\n─────────────────────────────────────────────────────")
